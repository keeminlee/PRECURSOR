...
> **Agent:** `.github/agents/Refocus.agent.md`
...
1. **`PLANS/`** — list all plan directories; for each, read the root plan file (`{slug}/{slug}.md`) to find step statuses
2. **`RECURSOR/PROJECTS.md`** — project registry for active project awareness. If this file does not exist, skip and note its absence in the output under Active Projects as "PROJECTS.md not found — run CONFIG_PROJECTS to generate."
...
(e.g., `@ExecuteRecurse execute PLANS/{slug}/{n}_{step-name}/`).}
```
