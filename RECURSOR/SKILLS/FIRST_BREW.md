...
**Prerequisite:** BOOTSTRAP must have been run successfully. All skills are installed in `RECURSOR/SKILLS/` and all agents are registered in `.github/agents/`.
...
This invokes the RECURSOR Start agent. Here is what happens behind the scenes:

1. **Context rehydration** — `@Start` reads `RECURSOR/PROJECTS.md` to discover all projects and their key files. If `RECURSOR/PROJECTS.md` does not exist yet, `@Start` automatically runs CONFIG_PROJECTS first (see Phase B).
...
If CONFIG_PROJECTS did not run (e.g., `RECURSOR/PROJECTS.md` already existed from a prior attempt), you can run it explicitly:

```
Run CONFIG_PROJECTS
```

### What is RECURSOR/PROJECTS.md?

`RECURSOR/PROJECTS.md` is the centralized inventory of every project and subrepo in your workspace. It contains:
...
Review the output. Edit `RECURSOR/PROJECTS.md` directly to fix anything, or re-run CONFIG_PROJECTS.

---

## Phase C — Create Your First Custom Agent

Agents in RECURSOR are `.agent.md` files in `.github/agents/`. Each agent is a thin wrapper: YAML frontmatter for metadata, then a markdown body with instructions.
...
Agents are thin wrappers. If a behavior is reusable across multiple contexts, put it in a skill file under `RECURSOR/SKILLS/` and have the agent read the skill. This keeps agents lightweight and skills composable.

---

## Phase D — The Daily Loop

RECURSOR operates on a daily rhythm with two bookends:
...
- **`@PlanRecurse`** — decomposes a task into a step tree under `PLANS/`. Does not write code.
- **`@ExecuteRecurse`** — implements a single step from a plan tree. Writes code and a PROGRESS.md audit trail.
- **`@CloseoutRecurse`** — writes a CLOSEOUT.md for a completed step and propagates status up the plan tree.
- **`@Refocus`** — mid-session reorientation. Read-only scan of current state, outputs a summary in chat. Use when you lose track of where you are.
- **Your custom agents** — whatever you built in Phase C and beyond.

For multi-step work, the flow is: `@PlanRecurse` → `@ExecuteRecurse` (per step) → `@CloseoutRecurse` (per step).
...
- [ ] `RECURSOR/PROJECTS.md` exists and has been verified
