...
2. `RECURSOR/PROJECTS.md` — project registry for ACTIVE project awareness
3. For each ACTIVE project: priority stack and implementation log (latest entries)

### Git activity read (after required reads)
For each ACTIVE project listed in `RECURSOR/PROJECTS.md`:
...
- `MUFG/packets/{date}/sod_outbound/` — formal MUFG outbound plan for today, if transmitted
...
2. **Open plan steps** — steps with status NOT STARTED or PARTIAL in any `PLANS/` tree,
...
For each suggested task, emit:
- Rank number
- Source tag: `[carryover]`, `[plan-step]`, `[git-momentum]`, or `[git-stale]`
- One-sentence description
- Project label
- File anchor (if applicable)
```
...
- [ ] `docs/week_N/{date}/MORNING_COFFEE.md` written and covers all 6 sections
- [ ] Priority stack is ranked and includes file anchors
- [ ] EOD checklist is specific enough to use as a sign-off at day close
