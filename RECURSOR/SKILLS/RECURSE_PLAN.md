...
> **Agent:** `.github/agents/PlanRecurse.agent.md`
> **Flow:** **Plan** → `@ExecuteRecurse` → `@CloseoutRecurse`
...
Create and manage a recursive plan tree under `PLANS/`. Two primary modes:

- **PLAN** — decompose a task description into a tree of step specs
- **RECURSE** — expand a SPLIT-rated step into its own sub-plan tree

This skill produces `.md` files under `PLANS/` only. It never reads or writes project source code, config files, scripts, or anything outside `PLANS/`.
...
|---|---|---|
| PLAN | `{task description}` | `@PlanRecurse add retry logic to the backend` |
| PLAN (slug override) | `slug={name} {task description}` | `@PlanRecurse slug=b360-retry add retry logic` |
| RECURSE | `recurse PLANS/{slug}/{n}_{step-name}/` | `@PlanRecurse recurse PLANS/b360-retry/2_add-backoff/` |
...
>
> **If you catch yourself creating `PLANS/{slug}/{n}_{step-name}/{m}_{sub-step}/` during PLAN mode — stop. That is RECURSE, not PLAN.**

**Read (in parallel):**
- `RECURSOR/PROJECTS.md` — understand what projects and files exist
...
- After writing, display the steps table inline in chat as a confirmation summary, then end with:
  > *"Plan ready. To implement: `@ExecuteRecurse execute PLANS/{slug}/{n}_{step-name}/`"*
...
**Read:**
- `PLANS/{slug}/{n}_{step-name}/{n}_{step-name}.md` — the target SPLIT step spec
- Codebase files listed in the step's Inputs
- Parent root plan file for context

**Write (in-place expansion):**
- **Append** a `## Sub-plan` section to `PLANS/{slug}/{n}_{step-name}/{n}_{step-name}.md` — the existing step spec. Do NOT create a separate sub-plan root file. The original sections (What, Inputs, Outputs, Criteria, Reasoning) stay untouched above. The appended section contains:
...
- `PLANS/{slug}/{n}_{step-name}/{m}_{sub-step}/{m}_{sub-step}.md` — one spec per sub-step, as direct children of the SPLIT step directory
...
- After writing, display the sub-steps table and end with:
  > *"Sub-plan ready. To implement: `@ExecuteRecurse execute PLANS/{slug}/{n}_{step-name}/{m}_{sub-step}/`"*
